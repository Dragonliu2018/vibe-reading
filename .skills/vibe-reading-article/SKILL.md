...
description: >
  ...
  Also use as the routing entry when the user explicitly asks for a private,
  local-only, or Corvus article; private rules live in the Corvus repository.
---
...
用户明确要求私有、local-only、Corvus 文章，或目标路径在 `_private/` 时：

1. 不执行下方公开文章流程，也不要写入 `_md/`。
2. 读取 `src/pages/articles/_private/.skills/corvus-article/SKILL.md`；若文件不存在，先运行 `npm run setup:private`。
3. 由 Corvus Skill 继续选择专题 Skill 并完成写作、检查和提交。

私有文章的路径、frontmatter、模板、图片和版本管理规则全部由私有仓库维护；不要在本 Skill 中复制或推断这些规则。
