...
基于 Step 1 扫描结果，用**四信号法**产生候选集，再用**分量门槛**过滤出有效模块数 M，按**根号原则**决定单层/两层结构——模块数由项目客观分化决定，不由人工设定：
...
- M ≤ 7 → **单层**：概览 + M 个模块文件
- M > 7 → **两层**：概览 + K 个子系统文件 + M 个模块文件（每个子系统平均挂 M/K ≈ √M 个模块）

概览文件**永远只列顶层**（K 个子系统 或 M 个模块），顶层始终 ≤ 7，符合工作记忆。读者从概览导航进入，两层结构时再进子系统——每层 ≤ 7，不会过载。
...
> 单层时顶层 = M 个模块直接展示；两层时顶层 = K 个子系统（K = ⌈√M⌉）。无论 M 多大，概览顶层始终 ≤ 7。

> **为什么不硬定 3-5**：大型项目（如 K8s 有 20+ 核心组件）强压到 5 个会丢信息或把不相近的职责揉一起，违背"明确的职责边界"。根号原则让模块数跟随项目客观社区数自适应——小项目自然 3-5，大项目自然 8-12，巨型项目自动两层。mycli v2.10.0：52 社区 → 过滤后有效 M = 4 → 单层 4 模块；K8s 级项目：过滤后 M ≈ 12-16 → 两层 4 子系统。

> **社区聚合分工**：graphify 社区粒度太细（如 mycli v2.10.0 有 52 个社区），需聚合到有效模块数 M。`aggregate-modules.py` 做一级聚类（按社区 local hub 的 source_file 归模块草案 + 输出 M 和根号分层建议），LLM 做二级语义合并——把同职责的社区组归并（如 `special/` 下的 iocommands/dbcommands/favoritequeries 子社区合成"特殊命令"模块）+ 命名 + 边界微调 + 按 K 分子系统（M > 7 时）。一级是图论（通用、确定性），二级是语义（项目相关、需判断）。
...
9. **图例与标签定位**
   ...
   - **标签之间也要 x/y 错开**——两个标签 y 相近（< 16px）时必须 x 错开：一个用 `text-anchor="end"` 放箭头左侧、一个 `start` 放右侧，或分置不同水平段。y 相近且 x 重叠是最常见的标签重叠来源
    - 对角线箭头的标签更难放置——优先用水平/垂直箭头，避免对角线
    - 标签不仅要避让箭头线（≥8px），还要避让相邻实线框——标签不能延伸到任何框的边界内
    - 当标签在两列间隙时，用 `text-anchor` 控制方向：右对齐用 `text-anchor="end"`，防止文字延伸到右侧框内
    - 对角线箭头的标签更难放置——优先用水平/垂直箭头，避免对角线
...
├── 02-{module-b}.md        # 模块 B
├── 03-{module-c}.md        # 模块 C
└── 04-{module-d}.md        # 模块 D（如有）
```

两层结构（M > 7，K = ⌈√M⌉ 子系统）——**扁平前缀命名，不建子目录**（现有内容集合无嵌套先例，扁平更安全，排序也天然正确）：

```
src/pages/articles/_md/{category_path}/
├── 00-overview.md                    # 概览（只列 K 个子系统，顶层导航）
├── 01-{subsystem-a}.md               # 子系统 A 概览（定位 + 挂载模块地图）
├── 01-{subsystem-a}-01-{module}.md   # 子系统 A 的模块 1
├── 01-{subsystem-a}-02-{module}.md   # 子系统 A 的模块 2
├── 02-{subsystem-b}.md               # 子系统 B 概览
├── 02-{subsystem-b}-01-{module}.md
└── ...
```

> 扁平前缀排序天然正确：`01-sub-a` < `01-sub-a-01-mod` < `01-sub-b`（短串在前，同前缀按后缀字母序）。两层时概览文件仍只展示 K 个子系统（≤7），读者从概览进子系统概览再进模块——每层 ≤7。子系统概览文件职责：定位子系统 + 列挂载模块 + 子系统内模块关系，不重复模块细节。
...
| 识别核心模块（凭经验） | Step 2 四信号法 + 分量门槛过滤 + 根号原则分层，程式化替代直觉 |
...
- Step 3：有效模块 M 个 + 数据流追踪 1 个 = **M+1 个 Agent**（M 由 Step 2 根号原则决定，小项目 4-6、大型 9-13）
