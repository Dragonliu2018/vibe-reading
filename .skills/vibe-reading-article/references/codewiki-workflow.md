...
基于 Step 1 扫描结果，用**四信号法**产生候选集，再用**分量门槛**过滤出有效模块数 M，按**复杂度原则**决定单层/两层结构——模块数由项目客观分化决定，不由人工设定：
...
**默认单层**：概览 + M 个模块文件。模块数 M 不设上限——M 由项目客观职责分化决定，小项目自然 3-5，大项目自然 8-12，概览模块地图表全部列出即可。

**两层仅当某模块特别复杂**：当某个模块自身分量过大（如独立子系统、跨多目录、需拆成多个子文件才能讲清），才对该模块做两层拆分——为它写一个子系统概览文件 + 若干子模块文件。这是**按复杂度拆分单个模块**，不是按总数触发全局两层。

| 情况 | 结构 | 触发条件 |
|---|---|---|
| 所有模块分量均衡 | **单层**：概览 + M 个模块 | 默认 |
| 某模块特别复杂 | 该模块两层（子系统概览 + 子模块），其余仍单层 | 该模块需拆成多文件才能讲清 |

> **为什么不用根号原则**：模块数是项目客观职责分化的结果，不该用 `√M` 人为压成两层。把不相近的职责揉进同一个子系统只为凑数，违背"明确的职责边界"。只有当**单个模块**复杂到需要自身分解（如 K8s 的 scheduler 子系统含多个独立组件），才对它做两层——拆分服务于"讲得清"，不服务于"数量好看"。
...
某模块做两层拆分时，该模块在表中标「子系统」，并链接到其子系统概览文件。
...
两层结构（仅当某模块特别复杂需拆分）——**扁平前缀命名，不建子目录**（现有内容集合无嵌套先例，扁平更安全，排序也天然正确）：

```
src/pages/articles/_md/{category_path}/
├── 00-overview.md                    # 概览（列全部模块，复杂模块链接到其子系统概览）
├── 01-{module-a}.md                  # 普通模块（单文件）
├── 02-{subsystem}.md                 # 复杂模块的子系统概览（定位 + 挂载子模块地图）
├── 02-{subsystem}-01-{sub-module}.md # 子系统子模块 1
├── 02-{subsystem}-02-{sub-module}.md # 子系统子模块 2
└── 03-{module-b}.md                  # 其余普通模块
```

> 扁平前缀排序天然正确：`02-sub` < `02-sub-01-mod` < `03-mod-b`（短串在前，同前缀按后缀字母序）。两层只针对复杂模块，其余模块仍单文件。子系统概览文件职责：定位子系统 + 列挂载子模块 + 子模块关系，不重复子模块细节。
