...
**4.2 模块关系图（SVG）**：从各 Agent 的"模块间交互"信息汇总，画出模块间的依赖关系，生成 `module-dependencies.svg`。用于概览「系统架构图」子节——展示横向组件间的依赖和调用关系。与 4.1 分层架构图视角不同：4.1 讲纵向分层，4.2 讲横向依赖。
...
5. 架构设计解析是否含架构图 + 分层设计（先讲思想 + 目录→层映射表）+ 设计模式 + 核心概念（如有）？→ 架构设计解析
6. 运行时行为是否含启动流程（含配置加载与对象装配）+ 核心运行流程（2-3 条主链路 + 业务↔代码↔数据流映射）+ 状态流（如有）？→ 运行时行为
7. 每个模块文件是否有模块定位 + 调用链路 + 核心类 + 设计决策 + 扩展方式？→ 模块文件
