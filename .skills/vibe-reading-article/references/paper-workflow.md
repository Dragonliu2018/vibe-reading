...
**Step 4 · 撰写文章（§1–§8）**：写到 `src/pages/articles/_md/{category_path}/<slug>.md`（`{category_path}` 规则见 `markdown-style.md`「文件命名」，含空格元素替换为 `-`），骨架见下文「文章结构 §1-§8」。导言引用块**首字段为 PDF 预览链接**；§3 必有原理图、§6 必有主结果图、其他重要图按归属入节；公式用 KaTeX 多行 `$$`（见「公式」节）；末尾放《摘要》折叠块（原文 + 译）。
...
## 9. 相关阅读
关联博客内其他相关文章（必加）。格式与关系标签词表见 `content-guide.md`「通用写作要点·相关阅读」；候选扫描方法见下文「相关阅读生成」。
```
...
- **抽图与用图必须对账**：抽图时按"宁多勿少"多抽几张候选可以，但博客写完后**必须回头核对**——子模块里的图与正文 `![...]()` 引用的图是否一一对应。多抽却没引用的图是垃圾资产，要么补进正文（若重要），要么从 `public/images/articles/{slug}/` 删除（若次要/重复）。对账命令：`diff <(ls public/images/articles/{slug}/) <(grep -o 'fig-[0-9]*-[a-z-]*\.png' src/pages/articles/_md/{slug}.md | sort -u)` 应无差异。
- **从 PDF 抽图**（PyMuPDF，已验证可用）。核心原则：**clip 紧贴图本身的 bbox，排除 caption 文本与页面装饰线**，否则会把正文/caption/页眉页脚一起框进去。
  ...
  **关键坑（实测）**：
  ...
  - **clip 四边都要有依据，不能只盯 caption 的 y0**：图本身是矢量/位图，不在 `get_text` 输出里——文字块坐标只能定位 caption（作下界），图的实际上/左/右边界必须交叉验证 `get_drawings()` / `get_images()` 的渲染元素 bbox。只看文本块就拍 clip，容易把"看起来覆盖图区"的整页范围当 clip，框进正文或腰斩标签。
  - **并排布局要按图的实际 x 范围收 clip**：论文常见"左文右图"（如甜甜圈图占页面右半 x≈345-540，左半 x≈55-333 是 §4 正文）。这种布局下 clip 的 x0 必须落在图的左边界（如 345）而非页边（55），否则会把左侧正文当图截进来。同理图在上、正文在下时要按 y 收紧。
  - **clip 底边逐行核对图的最底元素**：图内元素（如甜甜圈图的类别标签）可能延伸到比"直觉值"更靠下的位置。设底边前要核对 `get_text("blocks")` 列表里图区最底一个标签的 y1（或 drawings/images 的最大 y1），确保 clip 底边落在它下方、但在 caption y0 之上——否则标签被腰斩（底边太低）或 caption 被框入（底边太高）。留 2-5px 余量即可。

  **矩阵缩放**：`Matrix(3, 3)`（3× zoom）输出清晰；`220/72` 偏小。大图可降到 `Matrix(2, 2)`。

  **抽完即看一眼**：写完 clip 落盘后，**必须用 Read 打开结果图肉眼确认**——图是否完整、有无正文/ caption 混入、有无标签被腰斩。文字块坐标推断有盲区（图区几何、并排布局、标签延伸），只有看图才能兜底。不要等交付后被打回才复查。
...
文章主体（§1-§8）写完后、合规检查（Step 6）前，生成 §9 相关阅读：

1. **扫描候选**——在 `src/pages/articles/_md/` 里按以下维度找关联文章：
   - 同 `source.project`（同团队/同项目，如 LongCat 家族）
   - 同 `category` 域（同领域，如都是 Image Model）
   - 关键词重叠（模型名、方法名如 GRPO/DPO/DiT、benchmark 名）
   - 方法论镜像（不同项目但相同方法，如 firered↔LongCat-Image 都是多阶段 RLHF 图像编辑）
2. **精选 3–5 条**——不贪多，优先"读者最可能想接着读的"。
3. **标关系标签**——每条用固定词表（同家族/方法论镜像/工程实现/同基准对照/前序/后续/背景知识），见 `content-guide.md`。
4. **链接格式**——内部链接 `/vibe-reading/articles/<slug>`（flat 文件）或 `/vibe-reading/articles/<category_path>/<slug>`（子目录文件），**不引外部链接**。

> 论文解读最常见的三类关联：① 同家族姊妹模型（同团队同栈）；② 方法论镜像（不同项目同方法，横向对照）；③ 工程实现（本篇设计 → 别人的 PR 落地文章）。
