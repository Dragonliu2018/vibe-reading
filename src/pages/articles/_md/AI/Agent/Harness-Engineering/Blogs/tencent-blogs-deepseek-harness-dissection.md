...
source:
  type: "article"
  project: "Tencent"
  url: "https://mp.weixin.qq.com/s/DeIty-Nn8tQvE4osy7_bpg"
  author: "chino（腾讯WXG 微信小店前端开发工程师）"
  site: "腾讯技术工程"
date: "2026-09-18T17:20:00+08:00"
category: [AI, Agent, "Harness Engineering", Blogs]
contentType: "Blogs"
tags: ["DeepSeek Harness", "Cordis", "插件化", "可逆副作用", "Fiber", "effect", "HMR", "Agent Harness", "Code Mode", "Codex"]
description: "腾讯工程师对 DeepSeek Harness 运行时架构的深度拆解：Cordis 插件框架的 Fiber 生命周期与 effect 撤销栈实现可逆副作用，ctx.<service> 的 Proxy 解析，preset 两层 scope 链，Code Mode 的 worker_threads 隔离选型，工具注册分层遮蔽算法，并与 Codex 的工程决策对比。"
readingTime: "30 min"
aiModel: "Claude Opus 5"
reviewed: false
---

> **原文** [DeepSeek Harness 拆解：一套能拼装的 Agent 架构](https://mp.weixin.qq.com/s/DeIty-Nn8tQvE4osy7_bpg) · **作者** chino · **来源** 腾讯技术工程 · **原文发布** 2026-08-14 · **转载** 2026-09-18

---

DeepSeek 发布了第一款 Agent 产品，DeepSeek Harness。这次的重点放在了运行时架构上，官方给的关键词是"everything is a plugin"，模型接入、工具执行、会话记录、循环本身、界面，全部走插件机制。

![](/vibe-reading/images/articles/tencent-blogs-deepseek-harness-dissection/dsh-overview.png)
...
```typescript title="Plugin 联合类型定义"
...
```typescript title="ctx.effect 用法"
ctx.effect(() => {
  // 做副作用：注册监听、开定时器、提供服务……
...
![](/vibe-reading/images/articles/tencent-blogs-deepseek-harness-dissection/clock-plugin-example.png)

```typescript title="会话日志插件"
...
```typescript title="服务解析：沿 Fiber 链查找"
let fiber = ctx.fiber
while (true) {
...
```typescript title="max-tokens 粘性状态"
...
```typescript title="executionMode 动态并行判定"
...
```typescript title="PresetTree 挂载"
...
```typescript title="bindScopeParent 影子路由表"
...
```text title="worker 与 host 的消息协议"
...
![](/vibe-reading/images/articles/tencent-blogs-deepseek-harness-dissection/agent-lifecycle-chain.png)
...
![](/vibe-reading/images/articles/tencent-blogs-deepseek-harness-dissection/hmr-crash.png)
...
- [DeepSeek Harness CodeWiki](/vibe-reading/articles/AI/Agent/Harness-Engineering/DeepSeek-Harness/CodeWiki/0.1.0-rc.5/00-overview) —— 本博客对该代码库的源码架构解读（11 篇）
- [从一次 LLM 调用到完整 Harness，Agent 到底经历了什么？](/vibe-reading/articles/AI/Agent/Harness-Engineering/Blogs/tencent-blogs-from-llm-call-to-agent-harness) —— 同公众号 Harness 系列前作
- [靠这10个优化点，我们把Multi-Agent工作流成本降了50%以上](/vibe-reading/articles/AI/Agent/Harness-Engineering/Blogs/tencent-blogs-multi-agent-cost-optimization) —— 同公众号 Multi-Agent 成本优化实践
