...
source:
  type: "article"
  project: "SGLang"
  url: "https://www.lmsys.org/blog/2026-08-05-glmImage-optimization"
  author: "Ascend Team"
  site: "LMSYS Blog"
...
> **原文** [Full-Stack Performance Optimization of AR+DiT in SGL-Diffusion](https://www.lmsys.org/blog/2026-08-05-glmImage-optimization) · **作者** Ascend Team · **来源** LMSYS Blog · **原文发布** 2026-08-05 · **中英对照·AI 译** 2026-08-07
...
> **译：** 用 SRT 替换 HF 后端以加速 AR 建模并解决并行策略冲突——AR 用 TP、DiT 用 SP 各自独立；通过动态批处理提升硬件利用率，并支持已完成图像的提前返回；实现一卡一去噪器的并行 DiT 执行，通过缓冲 AR 结果重叠 AR 与 DiT 工作流。

![*Figure 1: Performance comparison.*](/vibe-reading/images/articles/sglang-official-ar-dit-sgl-diffusion-optimization/fig-01-performance-result.png)
...
> **译：** 混合自回归-扩散（AR+DiT）生成是一个统一框架，将用于全局上下文的 AR 建模与用于局部细节精修的 DiT 方法相结合。它利用 AR transformer 捕获长距离依赖，同时 DiT 模型迭代地精修输出，确保提升的质量与效率。GLM-Image 是这一趋势的典型：一个 9B 视觉语言模型首先从文本提示自回归地生成语义先验 token，然后一个 7B DiT 在 30–50 步内将这些 token 去噪为高分辨率图像。这种"先规划后绘制"的设计在知识密集型和文字密集的视觉任务上取得了 SOTA 结果——如海报、信息图和精确排版——这些场景下端到端扩散模型往往表现不佳。
...
> **译：** 然而，在 SGLang 中高效地 serving 这类混合流水线暴露了一个根本矛盾。在原生部署中，AR encoder、DiT denoiser 和 VAE decoder 串联在单一的单体 worker 进程内，由此产生三个关键痛点：
...
> **译：** 1. **架构耦合。** AR 和 DiT 共享同一进程、权重加载生命周期和调度域。扩展某一阶段必然拖动另一阶段；无法独立配置资源。AR 本质上是 LLM-decode 工作负载——吞吐随 batch size 和张量并行（TP）扩展。DiT 去噪则相反，是大张量、逐图像计算，适合跨卡空间并行（SP），在 batch=1 时效率最高。同构部署必须选择单一策略，注定某一阶段总是次优的。
...
> **译：** 2. **并发下资源利用率低。** 没有动态批处理时，并发请求被串行处理。端到端延迟几乎随请求并发度线性增长，大量可用算力闲置。
...
> **译：** 3. **资源分配错配。** DiT 在每设备 batch=1 时达到最佳单请求延迟，但将所有设备捆绑进单一单体流水线后，即使优先目标是吞吐，DiT 也被迫以多卡空间并行配置运行，导致硬件容量利用不足。
...
> **译：** 为解决这些问题，我们贡献了三个渐进式 PR，将系统从单体演化为完全解耦的异构分布式架构：
...
> **译：** 注：PR #31320 尚未合并。此处采用的 commit SHA 为 [4e520bd](https://github.com/sgl-project/sglang/pull/31320/changes/4e520bdeb14c72e3e80ac97a2d0b8169bb9268f1)

![*Figure 2: The whole pipeline for our optimization.*](/vibe-reading/images/articles/sglang-official-ar-dit-sgl-diffusion-optimization/fig-02-whole-pipeline.png)
...
> **译：** 为降低单图像生成延迟，我们的分析促使我们用 SRT 替换 HF 后端，最终形成 PR #25381。它将 AR 阶段从 diffusion worker 进程中解耦为独立的 SRT 服务，使 AR 和 DiT 分别加载权重、拥有解耦的调度生命周期、可独立扩展。同时，AR server 现在可以自行配置 TP，不再受 DiT 的 SP 策略约束。

![*Figure 3: The comparison between Original and Target.*](/vibe-reading/images/articles/sglang-official-ar-dit-sgl-diffusion-optimization/fig-03-glm-image-ar.png)
...
> **译：** AR 视觉语言 encoder 作为标准 SGLang SRT 服务启动，由 Diffusion 流水线通过新的 `--srt-encoder-url` 选项远程调用。AR server 复用 SGLang 既有的多模态 `sglang serve` 能力（`srt/models/glm_image_vl.py` + `srt/multimodal/processors/glm_image.py`）；Diffusion 侧仅向 `GlmImageAR` 阶段添加一个 HTTP `/generate` 分支，`VisionLanguageEncoderLoader` 在设置了 `srt-encoder-url` 时只做一次 `/health` 检查并返回 URL，而非调用 `from_pretrained` 加载权重。这种"独立 server + HTTP"方式将"把 VLM 重写为 SRT"的巨大任务变为"复用既有基础设施 + 一次 HTTP 调用"，大幅降低耦合并最小化对 diffusion 流水线的侵入式改动。

**Performance gains** (please refer to [PR #25381 description](https://github.com/sgl-project/sglang/pull/25381) for reproducing):

> **译：** **性能收益**（复现请参考 [PR #25381 描述](https://github.com/sgl-project/sglang/pull/25381)）：
...
> **译：** **资源匹配对比：** `Decoupled SRT AR (DiT unchanged)` 行展示相对于 `monolithic baseline (HF AR backend)` 的**纯软件**加速。
...
> **译：** **组合扩展：** `4-NPU heterogeneous (TP=4 AR, SP=4 DiT)` 行展示相对于同一 `1-NPU monolithic baseline` 的**软件+硬件扩展组合**结果（软件解耦 + 4× NPU 并行）。
...
> **译：** 在 PR #25381 之前，由于 AR 和 DiT 被约束共享同一并行策略，数据仅适用于 4-NPU 异构配置。
...
> **译：** AR 阶段加速最为显著：单卡 122.8 s → 26.1 s，4 卡上降低 78.8%。即使在 TP=1 下，SRT 的图执行、连续批处理和内存复用也带来了相对于朴素 transformers `generate` −62.1% 的提升。值得注意的是，基线 2-NPU 配置用 SP 将去噪削减 46.7%，但 AR 实际变慢 4.1%——在旧路径下 AR 从 SP 中零收益甚至因通信开销而退化；只有 SRT 路径让 AR 真正利用多卡 TP。
...
> **译：** 分离之后，AR 和 DiT 仍然一次执行一个请求，因此高并发下延迟线性增长（issue #30634）。为提升多输入场景下的吞吐，我们跟进 PR #30683。它将并发请求打包进单次前向传播，消除串行执行导致的算力闲置。
...
> **译：** 1. **动态批处理适配**：SGL-Diffusion 已包含通用动态批处理基础设施（在 [PR #18764](https://github.com/sgl-project/sglang/pull/18764) 中引入）；我们的工作通过实现 `supports_dynamic_batching` 和 `supports_native_grouped_requests` 接口及关联流水线逻辑，将此能力扩展到 GLM-Image。评估后，我们仅对 AR 阶段应用批处理，因为 DiT 每步延迟随 batch size 成比例增长，无净吞吐收益。
...
> **译：** 2. **支持提前返回**：我们添加 `supports_sequential_dit_inference` 变量及相关函数，支持每张输出图像就绪时提前返回，而非等待整个 batch 完成。

**Performance gains** (please refer to [PR #30683 description](https://github.com/sgl-project/sglang/pull/30683) for reproducing):

> **译：** **性能收益**（复现请参考 [PR #30683 描述](https://github.com/sgl-project/sglang/pull/30683)）：
...
> **译：** ¹ 处理延迟从 batch 派发到单个请求完成测量。对于 BS4/BS8/BS16，数值表示 batch 内的延迟范围：第一个数字对应最快完成的请求，最后一个对应最慢的。额外的排队等待时间（本测试中 ≤14 ms）可忽略。
...
> **译：** 完全解耦两个阶段，使 AR 和 DiT 各自采用最适合的并行与部署策略。AR encoder 偏好大 batch + TP（面向吞吐）；DiT 去噪在单 NPU 上 batch=1 时延迟和吞吐均最优。
...
> **译：** 随后 #31320 引入异构拓扑：一个批量 AR server + 一组独立的 batch=1 去噪器。这在单节点场景下实现了最优的全系统硬件利用率。

![*Figure 4: Final Deployment Architecture Diagram.*](/vibe-reading/images/articles/sglang-official-ar-dit-sgl-diffusion-optimization/fig-04-fanout-architecture.png)
...
> **译：** SGL-Diffusion 提供通用解耦框架；PR #31320 将此框架适配到 GLM-Image 的两阶段拓扑，实现并行 DiT 执行和 AR 生成与去噪之间的流水线重叠。一个关键设计选择是：仅通过 ZMQ 传输请求元数据和 CPU 侧先验 token ID——不跨节点传输大张量、latent、embedding 或 GPU buffer——旨在保持低通信开销。

**Performance gains** (please refer to [PR #31320 description](https://github.com/sgl-project/sglang/pull/31320) for reproducing):

> **译：** **性能收益**（复现请参考 [PR #31320 描述](https://github.com/sgl-project/sglang/pull/31320)）：
...
> **译：** 我们感谢华为 Ascend NPU 团队持续贡献 GLM-Image 优化。特别致谢 Maksim Emelin（@[Makcum888e](https://github.com/Makcum888e)）、Artem Savkin（@[OrangeRedeng](https://github.com/OrangeRedeng)）、Egor Filimonov（@[ssshinigami](https://github.com/ssshinigami)）和 Liang Zhen（@[ping1jing2](https://github.com/ping1jing2)）。
>
> 我们还感谢来自 CMB 的 Yuefeng Wu（@[ChefWu551](https://github.com/ChefWu551)）和 Qianqian Zheng（@[AuFlow](https://github.com/AuFlow)）。他们在 Ascend 平台上为 GLM-Image 优化做出贡献，提升了稳定性和部署效率。
...
> **译：** 我们感谢更广泛的 SGLang 社区，包括 Xiaoyu Zhang（@[BBuf](https://github.com/BBuf)）的代码审查，以及 Yuhao Yang（@[yhyang201](https://github.com/yhyang201)）等贡献者的初始讨论（issue #20032）和实现（PR #18809）。
>
> 最后，我们感谢 SGLang 维护者和审查者的悉心指导，感谢 Zhipu AI 团队开源 GLM-Image 模型和权重，以及每一位为 SGL-Diffusion 做出贡献的人。
...
> **译：** 由于命令篇幅较长，完整复现步骤列于 [issue #33526](https://github.com/sgl-project/sglang/issues/33526)。
