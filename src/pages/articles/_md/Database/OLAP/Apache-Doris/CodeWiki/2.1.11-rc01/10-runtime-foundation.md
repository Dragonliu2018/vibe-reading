...
运行时基础是 `be/src/runtime/`（~3.4 万行）+ `common/`（~1.2 万行）+ `io/`（~1.5 万行），是 BE 全模块共享的底层数据结构与资源管理。核心是 `ExecEnv`（BE god class 服务定位器，50+ 管理器）、`MemTracker`（层级内存追踪防 OOM）、`FileSystem`（多后端存储抽象）、`RuntimeState`（每查询运行时状态）。独立成文是因为这些跨模块共享的底层数据结构与资源管理独立于执行与存储——`Block`/`Column` 贯穿全链路，`ExecEnv` 持有所有管理器统一生命周期。

## 模块架构

```
ExecEnv (runtime/exec_env.h:123) ── BE 服务定位器单例
   ├─ GetInstance() (:146)  ── static 局部变量
...
MemTracker (runtime/memory/mem_tracker.h:48)
   └─ MemTrackerLimiter final (mem_tracker_limiter.h:67)
        ...
        └─ mem_tracker_limiter_pool (1000 组分桶降锁争用)
   │
   ▼ 查询状态
RuntimeState (runtime/runtime_state.h:66)
   ├─ _exec_env / _query_mem_tracker
   └─ init_mem_trackers() (runtime_state.cpp:332)
   │
   ▼ 文件系统抽象
FileSystem (io/fs/file_system.h:66) ── Strategy 基类
   ├─ LocalFileSystem (local_file_system.h:34)
...
BE 启动 → ExecEnv 初始化（`exec_env_init.cpp:142`）：

```
ExecEnv::init() → _init() (exec_env_init.cpp:149)
  ├─ init_doris_metrics()
  ...
  ├─ init_pipeline_task_scheduler() (:318)  ── TaskScheduler + BlockedTaskScheduler + RFTimerQueue
  ...
  ├─ _storage_engine = new StorageEngine(options); _storage_engine->open(); start_bg_threads()
  └─ _s_ready = true

查询内存注册：
  RuntimeState 构造 (runtime_state.cpp:55)
    → _query_mem_tracker = query_mem_tracker ?? ctx->query_mem_tracker ?? init_mem_trackers()
  SCOPED_ATTACH_TASK(query_mem_tracker) (thread_context.h:47)
...
`ExecEnv`（`exec_env.h:123-487`）单例（`GetInstance()` 返回 static 局部变量，`:146`），50+ 个 inline accessor 对外提供全局服务指针。所有服务 raw pointer 存储（非 unique_ptr）——注释（`:451-454`）："we choose to use raw pointer...avoid introducing a large number of header files which slow down compilation"。`_init()` 按依赖序创建（先 Thrift client cache → LoadChannel → StorageEngine），`destroy()` 用 `SAFE_STOP`/`SAFE_DELETE` 逆序销毁（注释 `:640`："StorageEngine must be destoried before _page_no_cache_mem_tracker.reset"）。`#ifdef BE_TEST` 下提供 `set_storage_engine()` 等 setter 允许测试注入。
...
`MemTrackerLimiter`（`mem_tracker_limiter.h:67`）`final` 继承 `MemTracker`，增加 limit + GC。`try_consume`（`:108`）CAS 检查 limit（overcommit tracker 受 `enable_query_memory_overcommit` 控制）。`free_top_memory_query`（`:167`）按内存用量从大到小取消 Query 释放内存。`mem_tracker_limiter_pool`（`exec_env.h:181`）固定 1000 组（`MEM_TRACKER_GROUP_NUM=1000`），每组 `list<weak_ptr<MemTrackerLimiter>>` + 独立锁，分桶降锁争用。

**Orphan 兜底**：未 attach tracker 的线程默认计入 Orphan（`exec_env.h:373-378` 注释），保证任何线程内存分配不丢失追踪，Orphan consumption 应接近 0。`cache_consume` + `_untracked_mem`（`:268-280`）原子累积，超 `mem_tracker_consume_min_size_bytes` 才 flush，避免每次 malloc 原子写。

### FileSystem 多后端抽象 + bthread 兼容

`FileSystem`（`io/fs/file_system.h:66`）Strategy 基类，`FileSystemType` 枚举（LOCAL/S3/HDFS/BROKER）。继承层级：`LocalFileSystem`（final）+ `RemoteFileSystem`（中基，增加 `connect_impl`/`upload_impl`/`download_impl`）→ `BrokerFileSystem`/`S3FileSystem`/`HdfsFileSystem`。

`FILESYSTEM_M` 宏（`:34-49`）是核心设计：检查 `bthread_self() == 0`，若在 bthread 中则 `AsyncIO::run_task(task, _type)` 切换到非 bthread 线程执行——避免阻塞 IO 卡住 bthread 调度。`FileFactory`（`file_factory.h:66`）`create_file_reader()` 按 `system_type` switch 分发到各后端 reader。

## 设计模式

| 模式 | 位置 | 为什么用 |
| --- | --- | --- |
| 服务定位器 | `ExecEnv::GetInstance` in `exec_env.h:146` | 50+ 管理器统一生命周期，避免 static init fiasco |
...
**新增一种 FileSystem 后端**（如 Azure Blob）：新建 `be/src/io/fs/azure_file_system.h` 继承 `RemoteFileSystem`，实现 `create_file_impl`/`delete_file_impl`/`exists_impl`/`file_size_impl`/`list_impl`/`connect_impl`/`upload_impl`/`download_impl` 等；新建 `azure_file_reader.cpp`/`azure_file_writer.cpp` 实现 `FileReader`/`FileWriter`；在 `io/file_factory.cpp` 的 `create_file_reader()` 加 `FILE_AZURE` 分支；在 `file_system.h:52` 的 `FileSystemType` 枚举加 `AZURE`；在 `file_factory.h:113` 的 `convert_storage_type()` 加映射；FE 侧 thrift `TFileType`/`TStorageBackendType` 加枚举。对应测试：`be/test/io/`。
