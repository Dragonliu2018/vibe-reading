...
联邦数据源是 `fe/fe-core/src/main/java/org/apache/doris/datasource/`（~4.2 万行，224 文件），负责外部数据源联邦查询——Hive/Iceberg/Hudi/Paimon/ES/JDBC/MaxCompute 等。核心是 `ExternalCatalog` 抽象基类 + 三级懒缓存（Caffeine）减少远端元数据访问。独立成文是因为外部元数据获取昂贵且异构（HMS Thrift、ES REST、JDBC driver 各不同协议），需要独立缓存与抽象层。2.1 中 `InternalCatalog`（内部库表）也迁移到此包，统一在 `CatalogIf` 接口下。

## 模块架构

```
ExternalCatalog (datasource/ExternalCatalog.java:94) abstract
   implements CatalogIf<ExternalDatabase<? extends ExternalTable>>, Writable
   ├─ makeSureInitialized() (:236)  ── final synchronized 模板方法
   │    ├─ initLocalObjects() → initLocalObjectsImpl() (:218)  ── 抽象钩子
   │    └─ [useMetaCache] buildDbForInit() (:657)  ── switch(logType)
   ├─ listDatabaseNames() (:168)  ── 委托 metadataOps
   └─ metadataOps : ExternalMetadataOps
   │
   ▼ 继承体系
HMSExternalCatalog (hive/HMSExternalCatalog.java:60)     ── Hive Metastore
EsExternalCatalog (es/EsExternalCatalog.java)             ── Elasticsearch
JdbcExternalCatalog (jdbc/JdbcExternalCatalog.java)       ── 通用 JDBC
...
PaimonExternalCatalog (paimon/, abstract)
   ...
   │
   ▼ 三级懒缓存
Catalog 级: makeSureInitialized() (:236)  ── 首次访问才初始化客户端
DB/Table 级: MetaCache<T> (metacache/MetaCache.java:34)  ── Caffeine LoadingCache
...
CatalogMgr (CatalogMgr.java:84)  ── nameToCatalog/idToCatalog 管理 Multi-Catalog
```
...
```
SQL → CatalogMgr.getCatalog(name) [CatalogMgr.java]
  → ExternalCatalog.getDbNullable(dbName) [ExternalCatalog.java:499]
    → makeSureInitialized() (:236)                    ── 惰性初始化
  ...
  → ExternalTable.getFullSchema() [ExternalTable.java:144]
    → ExternalSchemaCache.getSchemaValue() [ExternalSchemaCache.java:41]
...
`makeSureInitialized()`（`ExternalCatalog.java:236`）是 `final synchronized` 模板方法，定义初始化骨架：先 `initLocalObjects()`（→抽象 `initLocalObjectsImpl()` 钩子 `:218`，子类填具体客户端初始化），再按 `useMetaCache` 分支。注释明确："Catalog can't be init when creating because the external catalog may depend on third system."

三级懒加载：
1. **Catalog 级**：`makeSureInitialized()` 首次调用才初始化外部客户端，expireAfterWrite=86400s（24h）
2. **DB/Table 级**：`MetaCache<T>`（`metacache/MetaCache.java:34`）用 Caffeine `LoadingCache`，`getMetaObj()` miss 触发 loader
3. **Schema 级**：`ExternalSchemaCache`（`:41`）用 Caffeine `LoadingCache<SchemaCacheKey, Optional<SchemaCacheValue>>`，`getSchemaValue()` miss 调 `loadSchema()`
...
`CatalogMgr`（`CatalogMgr.java:84`）用 `nameToCatalog`（`:97`）/`idToCatalog` 两个 Map 管理 Multi-Catalog，`internal` 是默认 `InternalCatalog`，其余均为 `ExternalCatalog` 子类。`CatalogFactory.createFromStmt()`（`:87`）按 `catalogType` switch 实例化，`InitCatalogLog.Type` 枚举（`InitCatalogLog.java:35`：HMS/ES/JDBC/ICEBERG/PAIMON/MAX_COMPUTE/TEST）决定 `buildDbForInit()`（`:657`）实例化哪种 `ExternalDatabase`。

### Master/非 Master 转发

当 `useMetaCache == false` 且当前非 Master 时，`makeSureInitialized()` 通过 `MasterCatalogExecutor.forward(id, -1)` 转发到 Master 等待 journal 回放——非缓存模式元数据变更需 EditLog 持久化保证多 FE 一致性。缓存模式下各节点独立缓存，无需 Master 协调。

## 设计模式

| 模式 | 位置 | 为什么用 |
| --- | --- | --- |
| Template Method | `makeSureInitialized` in `ExternalCatalog.java:236` | 统一初始化骨架，子类只填钩子 |
