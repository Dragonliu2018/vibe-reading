...
source:
  type: "article"
  project: "Doris"
  url: "https://doris.apache.org/zh-CN/docs/3.x/data-operate/import/import-way/log-storage-analysis"
  author: "Apache Doris"
...
> **原文** [日志存储与分析](https://doris.apache.org/zh-CN/docs/3.x/data-operate/import/import-way/log-storage-analysis) · **作者** Apache Doris · **来源** Apache Doris 官方文档（3.x）· **转载** 2026-08-03
...
完成资源评估后，可以开始部署 Apache Doris 集群，推荐在物理机及虚拟机环境中进行部署。手动部署集群，可参考 [手动部署](https://doris.apache.org/zh-CN/docs/3.x/install/standard-deployment)。
...
在 `fe/conf/fe.conf` 目录下找到 FE 的相关配置项，并按照以下表格，调整 FE 配置。更多关于 FE 配置项的信息，可参考 [FE 配置项](https://doris.apache.org/zh-CN/docs/3.x/admin-manual/config/fe-config)。
...
- 使用时间字段上的 Range 分区（`PARTITION BY RANGE(ts)`），并开启[动态分区](https://doris.apache.org/zh-CN/docs/3.x/table-design/data-partitioning/dynamic-partitioning/)（`"dynamic_partition.enable" = "true"`），按天自动管理分区。
...
更多关于分区分桶的信息，可参考[数据划分](https://doris.apache.org/zh-CN/docs/3.x/table-design/data-partitioning/data-bucketing)。
...
- 获取支持输出至 Apache Doris 的 Filebeat 二进制文件。可点此下载或者从 Apache Doris 源码编译。

- 配置 Filebeat。需配置以下参数：
    - filebeat_demo.yml：配置所采集日志的具体输入路径和输出到 Apache Doris 的设置。

```yaml title="filebeat_demo.yml"
# input
filebeat.inputs:
- type: log
  enabled: true
...
更多关于 Kafka 配置和使用的说明，可参考 [Routine Load](https://doris.apache.org/zh-CN/docs/3.x/data-operate/import/import-way/routine-load-manual)。

**使用自定义程序采集日志**

除了对接常用的日志采集器以外，你也可以自定义程序，通过 HTTP API Stream Load 将日志数据导入 Apache Doris。参考以下代码：

```bash title="自定义程序采集日志"
...
```sql title="MATCH_ANY 全文检索"
SELECT * FROM your_table_name WHERE message MATCH_ANY 'error 404' ORDER BY ts DESC LIMIT 10;
```

- 检索请求字段中有 image 和 faq 的最新 10 条数据。其中，MATCH_ALL 是 Apache Doris 全文检索的 SQL 语法，用于匹配参数中所有关键字。

```sql title="MATCH_ALL 全文检索"
SELECT * FROM your_table_name WHERE message MATCH_ALL 'image faq' ORDER BY ts DESC LIMIT 10;
```
...
```sql title="MATCH_PHRASE 全文检索"
SELECT * FROM your_table_name WHERE message MATCH_PHRASE 'image faq' ORDER BY ts DESC LIMIT 10;
```
...
![可视化日志分析 WebUI](/vibe-reading/images/articles/doris-official-log-storage-analysis/webui.jpeg)
