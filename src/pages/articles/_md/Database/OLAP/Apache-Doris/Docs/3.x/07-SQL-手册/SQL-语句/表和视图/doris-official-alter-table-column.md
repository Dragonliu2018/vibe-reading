...
source:
  type: "article"
  project: "Doris"
  url: "https://doris.apache.org/zh-CN/docs/3.x/sql-manual/sql-statements/table-and-view/table/ALTER-TABLE-COLUMN"
  author: "Apache Doris"
...
> **原文** [ALTER TABLE COLUMN](https://doris.apache.org/zh-CN/docs/3.x/sql-manual/sql-statements/table-and-view/table/ALTER-TABLE-COLUMN) · **作者** Apache Doris · **来源** Apache Doris 官方文档（3.x）· **转载** 2026-08-04
...
> **提示** Doris 在建表之后有物化索引的概念，在建表成功后为 base 表，物化索引为 base index，基于 base 表可以创建 rollup index。其中 base index 和 rollup index 都是物化索引，在进行 schema change 操作时如果不指定 rollup_index_name 默认基于 base 表进行操作。
>
...
```sql title="修改列类型并移动位置"
ALTER TABLE example_db.my_table
MODIFY COLUMN col1 BIGINT KEY DEFAULT "1" AFTER col2;
```

> **提示** 无论是修改 key 列还是 value 列都需要声明完整的 column 信息
...
> **提示** 只能修改列的类型，列的其他属性维持原样

- 修改 Duplicate key 表 Key 列的某个字段的长度

```sql title="修改 Duplicate Key 列长度"
ALTER TABLE example_db.my_table
...
- 调整 example_db.my_table 的 key 列和 value 列的顺序（非聚合模型）
