...
source:
  type: "article"
  project: "Doris"
  url: "https://doris.apache.org/zh-CN/community/developer-guide/debug-tool"
  author: "Apache Doris"
...
> **原文** [FE/BE 调试工具](https://doris.apache.org/zh-CN/community/developer-guide/debug-tool) · **作者** Apache Doris · **来源** Apache Doris 官方文档 · **转载** 2026-07-31

---

Apache Doris 开发与运维中，经常需要对 FE 与 BE 进程进行调试。本文档汇总常用的 JVM 监控、堆内存分析、CPU 火焰图与内存合法性检查工具。

> 注：BE 二进制文件名 `doris_be`，在早期版本中为 `palo_be`。
...
FE 是 Java 进程，可使用 JDK 自带命令调试。

### 1. 统计当前内存使用明细

```bash title="统计 FE 内存使用"
...
Doris Debug Tools 提供了封装好的 CPU 火焰图与内存分析工具，下载页：[Releases](https://github.com/morningman/doris-debug-tools/releases)。注：非官方工具，仅用于开发调试。

### Jemalloc Heap Profile

> Apache Doris 1.2.2 版本开始默认使用 Jemalloc 作为内存分配器

Heap Profile 的原理可参考 [Heap Profiling 原理解析](https://cn.pingcap.com/blog/an-explanation-of-the-heap-profiling-principle/)。需要注意的是，Heap Profile 记录的是虚拟内存。Jemalloc 支持实时和定期两种 Heap Dump 方式，然后使用 `jeprof` 工具解析生成的 Heap Profile。

#### 1. 实时 Heap Dump（用于分析实时内存）

操作步骤：

1. 修改 `be.conf` 中 `JEMALLOC_CONF`：将 `prof:false` 改为 `prof:true`，将 `prof_active:false` 改为 `prof_active:true`
2. 重启 Doris BE
3. 在 BE 机器上调用 HTTP 接口生成 Heap Profile 文件：

   ```bash title="生成实时 Heap Profile"
...
| 触发方式 | 配置 | 说明 |
|----------|------|------|
| 内存累计申请达到阈值时 Dump | `lg_prof_interval=34` | 每次累计申请 16GB（`2^34 B`）时 Dump；2.1.6 之前默认就是 `32` |
...
```bash title="分析两个 Heap Profile 的 diff"
...
```bash title="升级 addr2line 到 2.35.2"
# 下载 addr2line 源码
...
```bash title="手动解析 Heap Profile 脚本"
#!/bin/bash
## @brief
## @author zhoufei
...
> Apache Doris 1.2.1 及之前版本使用 TCMalloc，1.2.2 版本开始默认使用 Jemalloc。如需切换回 TCMalloc，可使用 `USE_JEMALLOC=OFF sh build.sh --be` 进行编译。

使用 TCMalloc 时，遇到大内存申请会将堆栈打印到 `be.out`，形式如下：

```text title="TCMalloc 大内存申请日志"
tcmalloc: large alloc 1396277248 bytes == 0x3f3488000 @ 
 0x2af6f63 0x2c4095b 0x134d278 0x134bdcb 0x133d105 0x133d1d0 0x19930ed
```

表示 BE 在该堆栈上尝试申请 `1396277248 bytes` 内存。使用 `addr2line` 还原可读堆栈：

```bash title="还原 TCMalloc 堆栈"
addr2line -e lib/doris_be 0x2af6f63 0x2c4095b 0x134d278 0x134bdcb 0x133d105 0x133d1d0 0x19930ed
```

输出示例：

```text title="addr2line 输出示例"
...
```bash title="启用 TCMalloc Heap Profile"
...
```bash title="生成 CPU 火焰图"
pprof --svg --seconds=60 http://be_host:be_webport/pprof/profile > be.svg
```

该命令会生成 60 秒内 BE 执行的 CPU 消耗图：

![CPU Pprof](/vibe-reading/images/articles/doris-official-debug-tool/cpu-pprof.png)
...
2. 查看采集报告：
   ...
   ![Perf Report](/vibe-reading/images/articles/doris-official-debug-tool/perf-report.png)
...
3. 使用 FlameGraph 生成可视化火焰图：
   ...
   ![CPU Flame](/vibe-reading/images/articles/doris-official-debug-tool/cpu-flame.svg)
