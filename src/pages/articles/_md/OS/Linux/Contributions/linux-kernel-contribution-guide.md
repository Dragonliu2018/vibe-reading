...
commit message 内容（v1 首发版；v2 的演进见 9.4 节）：
...
# 9. 等 review → 改 → 发 V2（新顶层线程，绝不 --in-reply-to 挂 v1）
git commit --amend
git format-patch -o outgoing/ -v2 HEAD~1
# changelog（Changes in v2: ...）写在 patch 的 --- 之后、diffstat 之前
git send-email outgoing/v2-0001-*.patch --to ... --cc ...   # 不加 --in-reply-to
```
