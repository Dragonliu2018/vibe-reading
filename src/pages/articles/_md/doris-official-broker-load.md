...
source:
  type: "article"
  project: "Doris"
  url: "https://doris.apache.org/zh-CN/docs/3.x/data-operate/import/import-way/broker-load-manual"
  author: "Apache Doris"
...
> **原文** [Broker Load](https://doris.apache.org/zh-CN/docs/3.x/data-operate/import/import-way/broker-load-manual) · **作者** Apache Doris · **来源** Apache Doris 官方文档（3.x）· **转载** 2026-08-03
...
从 HDFS 或者 S3 直接读取，也可以通过 [湖仓一体/TVF](https://doris.apache.org/zh-CN/docs/3.x/data-operate/import/file-format/csv#tvf-导入) 中的 HDFS TVF 或者 S3 TVF 进行导入。基于 TVF 的 Insert Into 当前为同步导入，Broker Load 是一个异步的导入方式。
...
![Broker Load 原理](/vibe-reading/images/articles/doris-official-broker-load/broker-load.png)
...
本节演示了一个 S3 Load 示例。具体的使用语法，请参考 SQL 手册中的 [Broker Load](https://doris.apache.org/zh-CN/docs/3.x/sql-manual/sql-statements/table-and-view/dml/LOAD/)。
...
- 创建 CSV 文件 brokerload_example.csv 文件存储在 S3 上，其内容如下：

```text title="brokerload_example.csv"
1,Emily,25
2,Benjamin,35
3,Olivia,28
4,Alexander,60
...
- 使用 Broker Load 从 S3 导入数据。其中 bucket 名称和 S3 认证信息要根据实际填写：

```sql title="Broker Load 从 S3 导入"
...
当 Broker load 作业状态不为 CANCELLED 或 FINISHED 时，可以被用户手动取消。取消时需要指定待取消导入任务的 Label。取消导入命令语法可参考 [CANCEL LOAD](https://doris.apache.org/zh-CN/docs/3.x/sql-manual/sql-statements/table-and-view/dml/CANCEL-LOAD/) 查看。
...
采用 Kerberos 认证方式，需要 [krb5.conf](https://web.mit.edu/kerberos/krb5-1.12/doc/krb5_conf.html) 文件，krb5.conf 文件包含 Kerberos 的配置信息，通常，应该将 krb5.conf 文件安装在目录 `/etc` 中。可以通过设置环境变量 `KRB5_CONFIG` 覆盖默认位置。krb5.conf 文件的内容示例如下：
...
> **提示** 时间包含 `%3A`。在 hdfs 路径中，不允许有 `:`，所有 `:` 会由 `%3A` 替换。
...
```sql title="指定 source_sequence 列"
...
```sql title="jsonpaths 与 column list 配合"
LOAD LABEL example_db.label10
(
    DATA INFILE("hdfs://host:port/input/file.json")
    INTO TABLE `my_table`
...
关于 Broker Load 使用的更多详细语法及最佳实践，请参阅 [Broker Load](https://doris.apache.org/zh-CN/docs/3.x/sql-manual/sql-statements/table-and-view/dml/LOAD/) 命令手册，你也可以在 MySQL 客户端命令行中输入 `HELP BROKER LOAD` 获取更多帮助信息。
