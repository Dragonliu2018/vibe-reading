...
source:
  type: "article"
  project: "Doris"
  url: "https://doris.apache.org/zh-CN/docs/3.x/sql-manual/sql-statements/table-and-view/table/CREATE-TABLE"
  author: "Apache Doris"
...
> **原文** [CREATE TABLE](https://doris.apache.org/zh-CN/docs/3.x/sql-manual/sql-statements/table-and-view/table/CREATE-TABLE) · **作者** Apache Doris · **来源** Apache Doris 官方文档（3.x）· **转载** 2026-08-03
...
[ <key_type> KEY (<key_cols>) [ CLUSTER BY (<cluster_cols>) ] ]
...
```sql title="分区定义"
partitions_definition :
    AUTO PARTITION BY RANGE(<auto_partition_function>(<auto_partition_arguments>)) <origin_partitions_definition>
...
<one_partition_definition>:
  ...
  | PARTITION [ IF NOT EXISTS ] <partition_name> VALUES [ <partition_lower_bound>, <partition_upper_bound>)
  | FROM <partition_lower_bound> TO <partition_upper_bound> INTERVAL <n> [ <datetime_unit> ]
  | PARTITION [ IF NOT EXISTS ] <partition_name> VALUES IN { (<partition_value> [, <partition_value> [ ... ] ]) | <partition_value> }
```
...
ROLLUP (
    -- 聚合定义
    <rollup_name> (<rollup_cols>) [ DUPLICATE KEY (<duplicate_cols>) ]
    -- 其他聚合定义
...
```sql title="CREATE TABLE LIKE 语法"
...
有关更多详细信息，请参阅[标识符要求](https://doris.apache.org/zh-CN/docs/3.x/sql-manual/basic-element/object-identifiers/)和[保留关键字](https://doris.apache.org/zh-CN/docs/3.x/sql-manual/basic-element/reserved-keywords/)。
...
有关可以为表列指定的数据类型的详细信息，请参阅[数据类型](https://doris.apache.org/zh-CN/docs/3.x/sql-manual/basic-element/sql-data-types/data-type-overview/)章节。
...
表的数据模型。可选值为 DUPLICATE（明细模型）、UNIQUE（主键模型）、AGGREGATE（聚合模型）。有关数据模型的详细信息，请参阅[数据模型](https://doris.apache.org/zh-CN/docs/3.x/table-design/data-model/overview/)章节。

**`<key_cols>`**

表的 key 列。Doris 中 Key 列必须是表的前 K 个列。单个 tablet 中的数据会按照这些列保持有序。关于 Key 的限制，以及如何选择 Key 列，请参阅[数据模型](https://doris.apache.org/zh-CN/docs/3.x/table-design/data-model/overview/)章节中的各个小节。
...
列的聚合方式。只有当表是聚合模型时可以使用。聚合方式的详细信息，请参阅[聚合模型](https://doris.apache.org/zh-CN/docs/3.x/table-design/data-model/aggregate/)章节。

### 分桶相关参数

**`<distribute_cols>` 和 `<bucket_count>`**

分桶列和分桶数。明细模型的分桶列可以是任意的列，聚合模型和主键模型的分桶列必须和 key 列保持一致。分桶数是任意的正整数。有关分桶的详细信息，请参阅[手动分桶](https://doris.apache.org/zh-CN/docs/3.x/sql-manual/table-design/data-partitioning/data-bucketing/#%E6%89%8B%E5%8A%A8%E8%AE%BE%E7%BD%AE%E5%88%86%E6%A1%B6%E6%95%B0)和[自动分桶](https://doris.apache.org/zh-CN/docs/3.x/sql-manual/table-design/data-partitioning/data-bucketing/#%E8%87%AA%E5%8A%A8%E8%AE%BE%E7%BD%AE%E5%88%86%E6%A1%B6%E6%95%B0)章节。
...
在导入数据时，Doris 会为在自增列上没有指定值的数据行分配一个表内唯一的值。`<col_auto_increment_start_value>` 指定自增列的起始值。自增列的详细信息，请参阅[自增列](https://doris.apache.org/zh-CN/docs/3.x/table-design/auto-increment/)章节。
...
- CURRENT_TIMESTAMP [(`<precision>`)]：只能是 datetime 类型时使用。用当前时间作为默认值。可以指定时间精度。
...
指定索引标识符（即名称）。在创建的表中必须唯一。有关标识符的更多详细信息，请参阅[标识符要求](https://doris.apache.org/zh-CN/docs/3.x/sql-manual/basic-element/object-identifiers/)和[保留关键字](https://doris.apache.org/zh-CN/docs/3.x/sql-manual/basic-element/reserved-keywords/)。
...
索引的属性。详细说明请参考[倒排索引](https://doris.apache.org/zh-CN/docs/3.x/table-design/index/inverted-index/)章节。

### 自动分区相关参数

有关分区的详细介绍，请参阅[自动分区](https://doris.apache.org/zh-CN/docs/3.x/table-design/data-partitioning/auto-partitioning/)章节。
...
Doris 支持 RANGE 分区和 LIST 分区。详情请参阅[手动分区](https://doris.apache.org/zh-CN/docs/3.x/table-design/data-partitioning/manual-partitioning/)章节。

**`<partition_name>`**

分区标识符（即名称）。在创建的表中必须唯一。有关标识符的更多详细信息，请参阅[标识符要求](https://doris.apache.org/zh-CN/docs/3.x/sql-manual/basic-element/object-identifiers/)和[保留关键字](https://doris.apache.org/zh-CN/docs/3.x/sql-manual/basic-element/reserved-keywords/)。
...
range 分区。分区数据范围从 `<partition_lower_bound>` 到 `<partition_value_list>`。每建个创建一个分区。
...
> **注意** rollup 可以创建的同步物化视图功能有限。已不再推荐使用。推荐使用独立的语句创建同步物化视图。详情请参阅 [CREATE MATERIALIZED VIEW](https://doris.apache.org/zh-CN/docs/3.x/sql-manual/sql-statements/table-and-view/sync-materialized-view/CREATE-MATERIALIZED-VIEW/) 语句和[同步物化视图](https://doris.apache.org/zh-CN/docs/3.x/query-acceleration/materialized-view/sync-materialized-view/)章节。

**`<rollup_name>`**

同步物化视图的标示符（即名称）。在创建的表中必须唯一。有关标识符的更多详细信息，请参阅[标识符要求](https://doris.apache.org/zh-CN/docs/3.x/sql-manual/basic-element/object-identifiers/)和[保留关键字](https://doris.apache.org/zh-CN/docs/3.x/sql-manual/basic-element/reserved-keywords/)。
...
| compaction_policy | 配置这个表的 Compaction 的合并策略，仅支持配置为 time_series 或者 size_based。time_series: 当 rowset 的磁盘体积积攒到一定大小时进行版本合并。合并后的 rowset 直接晋升到 base compaction 阶段。在时序场景持续导入的情况下有效降低 compact 的写入放大率。此策略将使用 time_series_compaction 为前缀的参数调整 Compaction 的执行 |
...
| 动态分区相关属性 | 动态分区相关参考[数据划分 - 动态分区](https://doris.apache.org/zh-CN/docs/3.x/table-design/data-partitioning/dynamic-partitioning/) |
| enable_unique_key_skip_bitmap_column | 是否在 Unique Merge-on-Write 表上开启灵活列更新功能。此属性只能在 Unique Merge-on-Write 表上开启。(自 3.1.0 版本起支持) |

## 权限控制

执行此 SQL 命令的[用户](https://doris.apache.org/zh-CN/docs/3.x/admin-manual/auth/authentication-and-authorization/)必须至少具有以下[权限](https://doris.apache.org/zh-CN/docs/3.x/admin-manual/auth/authentication-and-authorization/)：
...
- 需要先创建 s3 resource 和 storage policy，表才能关联迁移策略成功
...
```sql title="CREATE TABLE LIKE 示例"
