...
source:
  type: "article"
  project: "Doris"
  url: "https://doris.apache.org/zh-CN/docs/3.x/data-operate/import/import-way/insert-into-manual/"
  author: "Apache Doris"
...
> **原文** [Insert Into Select](https://doris.apache.org/zh-CN/docs/3.x/data-operate/import/import-way/insert-into-manual/) · **作者** Apache Doris · **来源** Apache Doris 官方文档（3.x）· **转载** 2026-08-03
...
- 与 Multi-Catalog 外部表机制进行结合，如通过 Multi-Catalog 映射 MySQL 或者 Hive 系统中的表，然后通过 INSERT INTO SELECT 语法将外部表中的数据导入到 Doris 表中存储。
...
详细语法可以参见 [INSERT INTO](https://doris.apache.org/zh-CN/docs/3.x/sql-manual/sql-statements/table-and-view/dml/INSERT/)。
...
- 使用任何方式向源表导入数据（这里以 INSERT INTO VALUES 为例）
...
```sql title="INSERT INTO SELECT 导入"
...
- 可以使用 JOB 异步执行 INSERT。
- 数据源可以是 tvf 或者 catalog 中的表。
...
```sql title="INSERT INTO SELECT 语法"
...
```sql title="SHOW LOAD 查看过滤行"
SHOW LOAD WHERE label="xxx";
```

返回结果中的 URL 可以用于查询错误的数据，具体见后面查看错误行小结。数据不可见是一个临时状态，这批数据最终是一定可见的。可以通过 SHOW TRANSACTION 语句查看这批数据的可见状态：

```sql title="SHOW TRANSACTION 查看可见状态"
...
- FE 配置参数 `insert_load_default_timeout_second`。
- 环境变量 `insert_timeout`。
...
```sql title="创建 Hive Catalog"
CREATE CATALOG hive PROPERTIES (
    'type'='hms',
    'hive.metastore.urils' = 'thrift://172.0.0.1:9083',
...
- 关于创建 Doris 表的详细说明，请参阅 [CREATE-TABLE](https://doris.apache.org/zh-CN/docs/3.x/sql-manual/sql-statements/table-and-view/table/CREATE-TABLE) 语法帮助。

- 导入数据（从 hive.db1.source_tbl 表导入到 target_tbl 表）

```sql title="从 Hive 导入数据"
...
```sql title="DESC FUNCTION 查看 S3 文件 Schema"
DESC FUNCTION s3 (
...
关于 Insert Into 使用的更多详细语法，请参阅 [INSERT INTO](https://doris.apache.org/zh-CN/docs/3.x/sql-manual/sql-statements/table-and-view/dml/INSERT/) 命令手册，也可以在 MySQL 客户端命令行下输入 `HELP INSERT` 获取更多帮助信息。
