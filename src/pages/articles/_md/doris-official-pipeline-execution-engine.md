...
source:
  type: "article"
  project: "Doris"
  url: "https://doris.apache.org/zh-CN/docs/3.x/query-acceleration/optimization-technology-principle/pipeline-execution-engine"
  author: "Apache Doris"
...
> **原文** [Pipeline 执行引擎](https://doris.apache.org/zh-CN/docs/3.x/query-acceleration/optimization-technology-principle/pipeline-execution-engine) · **作者** Apache Doris · **来源** Apache Doris 官方文档（3.x）· **转载** 2026-08-04

---

Doris 的并行执行模型是一种 Pipeline 执行模型，主要参考了 Hyper 论文中 Pipeline 的实现方式，Pipeline 执行模型能够充分释放多核 CPU 的计算能力，并对 Doris 的查询线程的数目进行限制，从而解决 Doris 的执行线程膨胀的问题。它的具体设计、实现和效果可以参阅 [DSIP-027](https://doris.apache.org/zh-CN/community/DSIP-027) 以及 [DSIP-035](https://doris.apache.org/zh-CN/community/DSIP-035)。
...
![逻辑计划](/vibe-reading/images/articles/doris-official-pipeline-execution-engine/pip_exec_1.png)
...
![物理计划](/vibe-reading/images/articles/doris-official-pipeline-execution-engine/pip_exec_2.png)
...
![Pipeline 执行](/vibe-reading/images/articles/doris-official-pipeline-execution-engine/pip_exec_3.png)
...
![Pipeline 结构](/vibe-reading/images/articles/doris-official-pipeline-execution-engine/pip_exec_4.png)
...
扫描数据是一个非常重的 IO 操作，它需要从本地磁盘读取大量的数据（如果是数据湖的场景，就需要从 HDFS 或者 S3 中读取，延时更长），需要比较多的时间。所以我们在 ScanOperator 中引入了并行扫描的技术，ScanOperator 会动态的生成多个 Scanner，每个 Scanner 扫描 100 万 -200 万行左右的数据，每个 Scanner 在做数据扫描时，完成相应的数据解压、过滤等计算任务，然后把数据发送给一个 DataQueue，供 ScanOperator 读取。

![Scan 并行化](/vibe-reading/images/articles/doris-official-pipeline-execution-engine/pip_exec_5.png)
...
![Local Exchange](/vibe-reading/images/articles/doris-official-pipeline-execution-engine/pip_exec_6.png)
...
![Local Exchange 前后对比](/vibe-reading/images/articles/doris-official-pipeline-execution-engine/pip_exec_7.png)
